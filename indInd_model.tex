...
\begin{cases}
  \consLess{}{}{}{}{r}{\consLess{}{}{}{}{q}{p'}} & \text{where $m \leq n$} \\
  \consLess{}{}{}{}{q}{\stepindarg{p'}(x, r)} & \text{where $n \leq m$} 
\end{cases}
\]
...
0.2cm
\begin{tabular}{lcl}
$\gamma_{\Ctxt}$ &$=$& $\nilA \plOP \AindA(\one, \BindA(\one, (\lambda \oneelt .\,\inr(\star)),
\nilA)) : \SPAp$ \\
$\gamma_{\baseTybare}$ &$=$& $\AindB(\one, \nilB(\termAref(\inr(\oneelt))))$ \\
$\gamma_{\Pi}$ &$=$& $\AindB(\one, \BindB(\one, (\lambda \oneelt.\,\termAref(\inr(\star))),\BindB(\one,$ \\
...
\vskip 0.2cm
\begin{tabular}{l}
$\Pi : (\Gamma : \Ctxt) \to (A : \Ty(\Gamma)) \to \Ty(\consCtxt{\Gamma}{A}) \to
\Ty(\Gamma)$ \\
$\Pi(\Gamma, A, B) = \intro{B_{\gamma_{\Ctxt}, \gamma_{\Ty}}}(\langle \twoff, \langle (\lambda
    \oneelt.\Gamma), \langle (\lambda \oneelt.A),\langle (\lambda \oneelt. B) , \oneelt\rangle\rangle \rangle\rangle)$\enspace .
\end{tabular}
